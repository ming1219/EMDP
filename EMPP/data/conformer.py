...
class ConformerGen(object):
    ...
    def _init_features(self, **params):
        ...
        self.method = params.get('method', 'rdkit_etkdg')  # 默认使用新的ETKDG方法
    ...
    def single_process(self, smiles):
        ...
        """
        if self.method == 'rdkit_etkdg':
            # 添加明确的日志
            logger.debug(f'Generating conformer for {smiles} using ETKDG method')
            # 使用新的ETKDG+MMFF方法
            ...
            return feat, mol
        elif self.method == 'rdkit_random':
            # 使用原始方法
            atoms, coordinates, mol = inner_smi2coords_original(
                smiles, seed=self.seed, mode=self.mode, remove_hs=self.remove_hs
            )
            feat = coords2unimol(
                atoms,
            ...
            raise ValueError(
                'Unknown conformer generation method: {}. Supported methods: rdkit_etkdg, rdkit_random'.format(self.method)
            )
...
def inner_smi2coords_original(smi, seed=42, mode='fast', remove_hs=True, return_mol=False):
    '''
    Original function for converting SMILES to 3D coordinates using basic RDKit embedding.
    This function is kept for reference and comparison purposes.
    '''
...
def inner_smi2coords(smi, seed=42, mode='fast', remove_hs=True, return_mol=False):
    try:
        print(f"Processing SMILES: {smi} with ETKDG method")
        mol = Chem.MolFromSmiles(smi)
        if mol is None:
        ...
        try:
            ...
            params.maxAttempts = 5000 if mode == 'heavy' else 1000
            ...
            if res == 0:
                ...
                if coordinates_2d.shape[1] == 2:
                    z_noise = np.random.normal(0, 0.1, size=(len(coordinates_2d), 1))
                    coordinates = np.column_stack([coordinates_2d, z_noise.flatten()])
                else:
                    coordinates = coordinates_2d.copy()
                    coordinates[:, 2] += np.random.normal(0, 0.1, size=len(coordinates_2d))

        except Exception as e:
            # 使用更简单的调用方式作为备选
            try:
                res = AllChem.EmbedMolecule(mol, randomSeed=seed, maxAttempts=1000)
                if res == 0:
                    coordinates = mol.GetConformer().GetPositions().astype(np.float32)
                else:
                    AllChem.Compute2DCoords(mol)
                    ...
                    if coordinates.shape[1] == 2:
                        z_noise = np.random.normal(0, 0.1, size=(len(coordinates), 1))
                        coordinates = np.column_stack([coordinates, z_noise.flatten()])
                    else:
                        coordinates[:, 2] += np.random.normal(0, 0.1, size=len(coordinates))
            except:
                # 最终回退方案
                AllChem.Compute2DCoords(mol)
                coordinates = mol.GetConformer().GetPositions().astype(np.float32)
                if coordinates.shape[1] == 2:
                    coordinates = np.column_stack([coordinates, np.zeros((len(coordinates), 1))])

    except Exception as e:
        print(f"Failed to generate conformer for {smi}: {str(e)}, using 2D coordinates.")
        # 确保有合理的coordinates数组
        mol = Chem.MolFromSmiles('C' if smi.strip() == '' else smi)
        if mol is None:
...
def coords2unimol(
    atoms, coordinates, dictionary, max_atoms=256, remove_hs=True, energy=None, **params
):
    """
    ...
    :param energy: (float, optional) The energy of the conformer.
...
class UniMolV2Feature(object):
    ...
    def single_process(self, smiles):
        ...
        """
        if self.method == 'rdkit_etkdg':
            from .multi_conformer import multi_conformer_generation
            ...
            for coords, energy in coords_energy_list:
                ...
                feat = mol2unimolv2(temp_mol, self.max_atoms, remove_hs=self.remove_hs)
                if energy is not None:
                    feat['src_energies'] = float(energy)
                feats.append(feat)
            return feats, mol
        elif self.method == 'rdkit_random':
            # 使用原始方法
            mol = inner_smi2coords_original(
                smiles,
                seed=self.seed,
                mode=self.mode,
                remove_hs=self.remove_hs,
        ...
        if inputs and isinstance(inputs[0], list):
            ...
            for item in inputs:
                if item and len(item) > 0:
                    failed_conf.append((item[0]['src_coord'] == 0.0).all())
                else:
                    failed_conf.append(True)
            ...
            for item in inputs:
                if item and len(item) > 0:
                    failed_conf_3d.append((item[0]['src_coord'][:, 2] == 0.0).all())
                else:
                    failed_conf_3d.append(True)
        else:
            failed_conf = [(item['src_coord'] == 0.0).all() for item in inputs]
            logger.info(
                'Succeeded in generating conformers for {:.2f}% of molecules.'.format(
