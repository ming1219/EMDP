...
class MolDataReader(object):
    ...
    def read_data(self, data=None, is_train=True, **params):
        ...
        if isinstance(data, str):
            self.data_path = data
            if data.endswith('.sdf'):
                if PandasTools is None:
                    raise ImportError("PandasTools not available. Install X11 libraries or use CSV format.")
                data = PandasTools.LoadSDF(data)
            elif data.endswith('.csv'):
                # 添加自定义CSV读取逻辑
                data = pd.read_csv(self.data_path)
        ...
        if task == 'repr':
            ...
            if is_train:
                if anomaly_clean and task == 'regression':  # 仅对回归任务进行异常值清洗
