...
class Trainer(object):
    ...
    def _init_trainer(self, **params):
        ...
        self.max_epochs = params.get('epochs', 300)
    ...
    def _init_wandb(self, **params):
        self.wandb_run = None
        self.use_wandb = bool(params.get('use_wandb', False))
        ...
        try:
            ...
            cfg = {
                ...
                'metrics': getattr(self, 'metrics_str', None),
                ...
                'model_size': params.get('model_size', None),
                'kfold': params.get('kfold', None),
            }
            project = params.get('wandb_project', None)
            entity = params.get('wandb_entity', None)
            name = params.get('wandb_run_name', None)
            tags = params.get('wandb_tags', None)
            if project is not None:
                self.wandb_run = wandb.init(project=project, entity=entity, name=name, config=cfg, tags=tags, reinit=True)
            else:
                self.wandb_run = wandb.init(name=name, config=cfg, tags=tags, reinit=True)
            logger.info('wandb initialized')
        except Exception as e:
            logger.error(f'wandb init failed: {str(e)}')
            raise

    def _wandb_log(self, epoch, total_trn_loss, total_val_loss, metric_score, optimizer, fold=None):
        ...
        data = {
            'epoch': epoch + 1,
            'train_loss': float(total_trn_loss),
            'val_loss': float(total_val_loss),
            f'val_{_metric}': float(_score),
            'lr': float(optimizer.param_groups[0]['lr']),
        }
        if fold is not None:
            data['fold'] = fold
        self._wandb_module.log(data)
    ...
    def fit_predict_wo_ddp(
        ...
        # Move loss_func to device if it's an nn.Module (e.g., UncertaintyWeightedLoss)
        ...
        for epoch in range(self.max_epochs):
            ...
            self._log_epoch_results(
                epoch, total_trn_loss, np.mean(val_loss), metric_score, optimizer, loss_func
            )
        ...
        # Log learned uncertainty weights if using UncertaintyWeightedLoss
        if isinstance(loss_func, nn.Module) and hasattr(loss_func, 'get_task_weights'):
            weights = loss_func.get_task_weights().cpu().numpy()
            uncertainties = loss_func.get_task_uncertainties().cpu().numpy()
            logger.info(f"Learned task weights (precision): {weights}")
            logger.info(f"Learned task uncertainties (sigma): {uncertainties}")
    ...
    def fit_predict_with_ddp(
        ...
        # Move loss_func to device if it's an nn.Module (e.g., UncertaintyWeightedLoss)
        if isinstance(loss_func, nn.Module):
            loss_func = loss_func.to(local_rank)
    ...
    def _initialize_optimizer_scheduler(self, model, train_dataloader, loss_func=None):
        num_training_steps = len(train_dataloader) * self.max_epochs
        num_warmup_steps = int(num_training_steps * self.warmup_ratio)
        
        # Build parameter groups: model parameters + loss function parameters (if any)
        param_groups = [{'params': model.parameters(), 'lr': self.learning_rate}]
        
        # Check if loss_func is an nn.Module with learnable parameters (e.g., UncertaintyWeightedLoss)
        if loss_func is not None and isinstance(loss_func, nn.Module):
            loss_params = list(loss_func.parameters())
            if len(loss_params) > 0:
                # Add loss function parameters with potentially different learning rate
                param_groups.append({
                    'params': loss_params,
                    'lr': self.learning_rate  # Can be adjusted if needed
                })
                logger.info(f"Added {len(loss_params)} loss function parameters to optimizer")
        
        optimizer = Adam(param_groups, eps=1e-6)
    ...
    def _compute_loss(self, model, net_input, net_target, loss_func):
        if self.scaler and self.device.type == 'cuda':
            ...
            # Compute loss outside autocast for nn.Module loss functions
            # to avoid dtype mismatch with learnable parameters
            if isinstance(loss_func, nn.Module):
    ...
    def _log_epoch_results(
        self, epoch, total_trn_loss, total_val_loss, metric_score, optimizer, loss_func=None
    ):
        _score = list(metric_score.values())[0]
        _metric = list(metric_score.keys())[0]
        ...
        if loss_func is not None and isinstance(loss_func, nn.Module) and hasattr(loss_func, 'get_task_weights'):
            weights = loss_func.get_task_weights().cpu().numpy()
            # Format weights as a compact string
            weights_str = ', '.join([f'{w:.3f}' for w in weights])
            logger.info(f'Task weights: [{weights_str}]')
        
        self._wandb_log(epoch, total_trn_loss, total_val_loss, metric_score, optimizer)
        return False
