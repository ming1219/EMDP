...
class UniMolV2Model(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        if 'K' in kwargs and 'src_energies' in kwargs and not return_repr:
            K = kwargs['K']
            K = K.item() if isinstance(K, torch.Tensor) else int(K)
            src_energies = kwargs['src_energies']
            if isinstance(src_energies, list):
                src_energies = torch.tensor(src_energies).to(cls_repr.device)
            else:
                src_energies = src_energies.to(cls_repr.device)

            N = cls_repr.size(0) // K
            dim = cls_repr.size(1)
            cls_repr = cls_repr.view(N, K, dim)

            src_energies = src_energies.view(N, K)

            beta = 1.0 / (GAS_CONSTANT_R * DEFAULT_TEMP)

            
            log_weights = -beta * src_energies
            weights = F.softmax(log_weights, dim=1).unsqueeze(-1)

            cls_repr = torch.sum(cls_repr * weights, dim=1)
    ...
    def batch_collate_fn(self, samples):
        """
        统一的 collate 函数，支持单构象和多构象输入。
        数据格式规范:
        - 输入: [(data, label), ...] 其中 data 可以是 dict 或 list[dict] 或 np.ndarray
        - 输出: (batch_dict, labels)
        处理流程:
        1. 规范化所有输入为 list[dict] 格式
        2. 展平多构象数据
        3. 统一 padding 处理
        """

        PAD_CONFIG = {
            'atom_feat':     (pad_coords,     {'dim': 8}),
            'atom_mask':     (pad_1d_tokens,  {}),
            'edge_feat':     (pad_2d,         {'dim': 3}),
            'shortest_path': (pad_2d,         {}),
            'degree':        (pad_1d_tokens,  {}),
            'pair_type':     (pad_2d,         {'dim': 2}),
            'attn_bias':     (pad_2d,         {}),
        ...
        def normalize_sample(sample):
            data, label = (sample[0], sample[1]) if isinstance(sample, (tuple, list)) and len(sample) >= 2 else (sample, None)
            if isinstance(data, np.ndarray):
                data = data.item() if data.ndim == 0 else data.tolist()
        ...
        K = len(normalized[0][0])
        flat_feats = []
        energies = []
        for feat_list, _ in normalized:
            for feat in feat_list:
                flat_feats.append(feat)
                energies.append(feat.get('src_energies', 0.0))
        batch = {}
        if K > 1:
            batch['K'] = torch.tensor(K, dtype=torch.int64)
            batch['src_energies'] = torch.tensor(energies, dtype=torch.float32)
        for key, (pad_fn, kwargs) in PAD_CONFIG.items():
            if key not in flat_feats[0]:
                continue
            tensors = [torch.tensor(f[key]) for f in flat_feats]
            batch[key] = pad_fn(tensors, pad_idx=self.padding_idx, **kwargs)
        try:
            label = torch.tensor([s[1] for s in normalized if s[1] is not None])
