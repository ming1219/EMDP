...
from .loss import FocalLossWithLogits, GHMC_Loss, MAEwithNan, myCrossEntropyLoss, UncertaintyWeightedLoss
...
LOSS_RREGISTER = {
    ...
    'multilabel_regression': {
        'mae': MAEwithNan,
        'uncertainty': UncertaintyWeightedLoss,  # Uncertainty weighted loss
    },
}
...
class NNModel(object):
    ...
    def __init__(self, data, trainer, **params):
        ...
        if self.task == 'multilabel_classification':
            ...
            if self.loss_key is None:
                self.loss_key = 'mae'  # Default to MAE for backward compatibility
            if self.loss_key == 'uncertainty':
                # Initialize UncertaintyWeightedLoss with num_tasks
                print(f"[DEBUG] Creating UncertaintyWeightedLoss with num_tasks={self.num_classes}")
                self.loss_func = LOSS_RREGISTER[self.task][self.loss_key](num_tasks=self.num_classes)
                print(f"[DEBUG] loss_func type: {type(self.loss_func)}")
                print(f"[DEBUG] loss_func is nn.Module: {isinstance(self.loss_func, nn.Module)}")
            else:
                self.loss_func = LOSS_RREGISTER[self.task][self.loss_key]
                print(f"[DEBUG] Using MAE loss_func type: {type(self.loss_func)}")
        else:
